...
public class Cart {
    ...
    func removeItem(name: String, count: Int) -> Bool {
        guard count > 0 else {
            print("❌ Ошибка: количество должно быть больше нуля")
            return false
        }
        
        //  замыкание item in item.product.name == name
        //  сокращенное замыкание $0 - первый параметр
        guard let index = items.firstIndex(where: { $0.product.name == name }) else {
            print("❌ Товар '\(name)' не найден в корзине")
            return false
        }
        
        let currentItem = items[index]
        let newCount = currentItem.count - count
        
        if newCount <= 0 {
            items.remove(at: index)
            print("✅ Товар '\(name)' полностью удален из корзины")
        } else {
            currentItem.count = newCount
            print("✅ Количество товара '\(name)' уменьшено до \(newCount)")
        }

...
func checkSecondFeat(_ catalog: Catalog) {
    ...
    let user: User = User(name: "Александр", age: 25, balance: 200000, cart: cart)
    var ok: Bool;
    print("\n=== ДОБАВЛЕНИЕ ТОВАРОВ ===\n")
    ok = user.cart.addItemCart(name: "MacBook Pro", count: 1)
    ok = user.cart.addItemCart(name: "Swift Programming", count: 2)
    ok = user.cart.addItemCart(name: "Клавиатура", count: 1)
    ok = user.cart.addItemCart(name: "Кофе", count: 20)
    ok = user.cart.addItemCart(name: "AirPods Pro", count: 0)

    print("\n=== КОРЗИНА ===\n")
    user.cart.printInfo()
}

checkSecondFeat(checkFirstFeat())
